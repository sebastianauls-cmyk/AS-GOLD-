...
export function compareCountryLegalModules(referenceRecord,targetRecord){
  ...
  return {version:COUNTRY_LEGAL_COMPARISON_VERSION,reference_country:{code:reference.key,label:reference.label,jurisdiction:reference.jurisdictionLabel},target_country:{code:target.key,label:target.label,jurisdiction:target.jurisdictionLabel},overall:light(severity),rows,rule:'Ampel bewertet Prüf- und Abdeckungsunterschiede und ersetzt keine inhaltliche Rechtsvergleichung. Neue rechtlich relevante Produktmodule werden automatisch zur Länderprüfungspflicht. Inhaltliche Rechtsunterschiede dürfen erst nach belegter Quellenprüfung als solche behauptet werden.'}
}
