...
export function LanguageSwitcher({value,onChange,label='Sprache',className='',showLabel=false,publicPicker=false,onExplainer=null}){
  ...
  useEffect(()=>{
    ...
    const apply=()=>setMobileMenu(mq.matches)
    apply()
  ...
  useEffect(()=>{
    ...
    const closeEscape=event=>{if(event.key==='Escape'){setOpen(false);rootRef.current?.querySelector('.flagLanguageTrigger')?.focus()}}
  ...
  const menu=open&&<>
    {mobileBackdrop}
    <div className="flagLanguageMenu" id={menuId} role="listbox" aria-label={label} ref={menuRef}>
      <button type="button" className="flagLanguageMenuBack" onClick={()=>{setOpen(false);rootRef.current?.querySelector('.flagLanguageTrigger')?.focus()}} aria-label={backLabel.replace(/^←\s*|\s*→$/g,'')}>{backLabel}</button>
      {supportedLanguages.map(item=><button type="button" role="option" aria-selected={item.key===value} aria-label={item.label} title={item.label} className={item.key===value?'active':''} onClick={()=>{onChange(item.key);setOpen(false);rootRef.current?.querySelector('.flagLanguageTrigger')?.focus()}} key={item.key}>
  ...
  // A fixed menu inside a filtered or scrolling header can be clipped on phones.
  // Keep its touch targets in a viewport-level layer, outside all header panels.
  const menuLayer=open&&mobileMenu?createPortal(<div className="flagLanguageOverlay" dir="ltr">{menu}</div>,document.body):menu
