...
export function composeDocumentLanguageWorkflowSummary(result={},outputLanguage='de',referenceLanguage='de'){
  ...
  // The original and formal letters already have their own fields. Preserve a
  // real translation, but do not repeat the same-language original as a result.
  const source=String(result.source_language||'').toLowerCase().split(/[-_]/)[0]
  if(result.document_translation&&source!==outputLanguage&&result.document_translation.trim()!==String(result.extracted_text||'').trim()) sections.push(`ÜBERSETZUNG DES ORIGINALDOKUMENTS (${language.label})\n${result.document_translation}`)
  return sections.join('\n\n────────────────────────\n\n')
}

export function readableDocumentSummary(value='',original=''){
  const text=String(value||'')
  // Only unpack the exact legacy container generated by ASH; arbitrary user
  // prose is never shortened. Originals and letters have dedicated fields.
  if(!text.includes('ERKLÄRUNG FÜR DEN KUNDEN (')||!text.includes('────────────────────────'))return text
  const sections=text.split(/\n\n────────────────────────\n\n/u)
  const explanation=sections.find(section=>section.startsWith('ERKLÄRUNG FÜR DEN KUNDEN ('))
  if(!explanation)return text
  const translation=sections.find(section=>section.startsWith('ÜBERSETZUNG DES ORIGINALDOKUMENTS ('))
  const normalize=value=>value.replace(/\s+/gu,' ').trim()
  const body=section=>section.slice(section.indexOf('\n')+1)
  return [body(explanation),translation&&normalize(body(translation))!==normalize(String(original||''))?translation:''].filter(Boolean).join('\n\n')
}
