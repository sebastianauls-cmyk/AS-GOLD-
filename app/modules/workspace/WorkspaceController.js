...
import { InternalChangeControlSurface } from '../team-account/InternalChangeControlSurface'
...
export default function WorkspaceController({publicOnly=false}={}){
  ...
  const pricingReturnCase=useRef(null)
  const resultContinuation={
    canContinue:access?.permissions?.full_analysis===true,
    currentTier,
    upgradeOptions:upgrades.map(upgrade=>({...localizedPlans.find(plan=>plan.key===upgrade.plan_key),...upgrade})),
    liveLocked:paymentConfig?.liveLocked!==false,
    onPlans:()=>{pricingReturnCase.current=selectedCase||data.cases.find(item=>item.id===selectedDocument?.case_id)||null;setSelectedDocument(null);setSelectedCase(null);setSelectedClient(null);setSection('pricing')},
