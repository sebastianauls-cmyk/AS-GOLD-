...
export function DocumentsSurface({a,access,documents,core,cases,documentMode,setDocumentMode,uploadCaseId,uploadDocument,uploading,allowedUploadAccept,setSelectedDocument,onBack}){
  return <>
    ...
    {documents.length?<div className="itemList">{documents.map(item=><button className="itemRow buttonRow" type="button" onClick={()=>setSelectedDocument(item)} key={item.id}><div><b>{item.title}</b><p>{item.document_type||core.documentType}{item.document_date?` · ${item.document_date}`:''}</p></div><span className="chev">›</span></button>)}</div>:null}
  </>
}
