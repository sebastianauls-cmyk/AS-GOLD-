...
export function TesterGuide(){
  return <LegalDocument pageId="testen" localizable showRelease eyebrow={`Testerbetrieb · ${APP_VERSION} Pilot & Abnahme`} title="ASH Workspace Gold sicher ausprobieren" intro="Hier prüfen externe Tester den vollständigen modularen Ablauf mit sicheren Testdaten. Ziel ist jetzt die kontrollierte Abnahme von Verständlichkeit, Zuverlässigkeit und Marktreife." updated={APP_RELEASE.updated} localizedExtra={<><GuestTestStartButton/><TesterShareButton/></>} localizedExtraAfterSection={0}>
    ...
    <LegalSection title="Pilot-Abnahme: drei feste Teststufen">
      <p>Jeder Tester sollte mindestens einen kleinen, einen mittleren und einen komplexen Ablauf durchführen. Entscheidend ist, ob ASH Workspace ohne zusätzliche Erklärung verständlich bleibt.</p>
    ...
    <LegalSection title="Synthetischer Musterfall"><p><a className="secondary btn" href="/testdaten/ASH_Workspace_Gold_Synthetischer_Testfall_V29.pdf">Musterdatei herunterladen</a></p><p>Die Musterdatei enthält keine echten personenbezogenen Daten und kann für Upload, Analyse, Ampel und Export verwendet werden.</p></LegalSection>
    <LegalSection title="Abnahmeentscheidung"><p><b>{APP_VERSION} gilt für den Pilot als bestanden, wenn kein reproduzierbarer 🔴 Blocker offen bleibt und wiederkehrende 🟡 Punkte dokumentiert und priorisiert sind.</b></p><p>🟢 Rückmeldungen bestätigen den vorgesehenen Ablauf und werden nicht automatisch zu neuen Entwicklungswünschen.</p></LegalSection>
    <LegalSection title="Testfeedback"><p><a className="primary btn" href={`mailto:sebastian.auls@gmail.com?subject=ASH%20Workspace%20Gold%20${APP_VERSION}%20Pilotfeedback&body=Ger%C3%A4t%3A%0ABrowser%3A%0ASprache%3A%0ATestfall%3A%0AEinstufung%20%28Rot%2FGelb%2FGr%C3%BCn%29%3A%0ABetroffener%20Schritt%3A%0ABeschreibung%3A`}>Pilotfeedback zu {APP_VERSION} senden</a></p><p>{APP_VERSION} ist ein kontrollierter Produkttest. Ergebnisse müssen vor einer verbindlichen Verwendung weiterhin geprüft und ausdrücklich freigegeben werden.</p></LegalSection>
  </LegalDocument>
}
