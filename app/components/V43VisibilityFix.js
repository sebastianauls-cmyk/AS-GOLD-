...
function reconcile(){
  ...
  if(!bar){
    ...
    bar.setAttribute('data-v48-fallback-controls','true')
...
export function V43VisibilityFix(){
  useEffect(()=>{
    ...
    if(!style){
      ...
      style.textContent=`#asgold-v43-visible-controls[hidden]{display:none!important}#asgold-v43-visible-controls{position:fixed;left:10px;right:10px;bottom:10px;z-index:9999;display:flex;gap:8px;justify-content:center;flex-wrap:wrap;padding:8px;border:1px solid #c9ad66;border-radius:14px;background:rgba(255,255,255,.97);box-shadow:0 8px 30px rgba(20,24,30,.18)}#asgold-v43-visible-controls button{min-height:46px;padding:9px 13px;border-radius:10px;border:1px solid #c9ad66;background:#fff;color:#2f291b;font-weight:900;font-size:.95rem}#asgold-v43-visible-controls [data-v48-video]{background:#2f291b;color:#fff}@media(min-width:900px){#asgold-v43-visible-controls{left:auto;right:18px;bottom:18px;width:auto}}`
