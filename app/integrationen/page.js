...
export default function Integrationen(){
  ...
  <section style={{marginTop:24,padding:20,background:'#fffaf0',border:'1px solid #e5d7b5',borderRadius:16}}>
    ...
    <ol style={{margin:'12px 0 0',paddingLeft:22,lineHeight:1.6}}>
      ...
      <li>Bei E-Mail oder Cloud melden Sie sich selbst direkt beim jeweiligen Anbieter an. AS Gold erhält Ihr Passwort nicht.</li>
      <li>Der Anbieter zeigt Ihnen, auf welche Funktionen AS Gold zugreifen möchte. Sie entscheiden selbst, ob Sie diese Freigabe erteilen.</li>
      <li>Nach erfolgreicher Freigabe kann AS Gold nur die ausdrücklich erlaubten Funktionen nutzen, zum Beispiel E-Mails lesen, Anhänge übernehmen, Antwortschreiben versenden oder Dateien speichern.</li>
    ...
    <p style={{margin:'12px 0 0'}}><b>Sie müssen keine technischen Zugangsdaten eingeben.</b> Die normale Anmeldung bei Ihrem E-Mail- oder Cloud-Anbieter und Ihre Zustimmung reichen aus, sobald der jeweilige Dienst für AS Gold freigeschaltet ist.</p>
  </section>

  <section style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:14,marginTop:28}}>
    <article style={card}><b>✉ Gmail</b><p>E-Mails lesen, Anhänge übernehmen und vorbereitete Antwortschreiben über das verbundene Konto versenden.</p><div style={infoBox}><b>Was Sie tun</b><span>„Gmail verbinden“ wählen, sich bei Google anmelden und die angezeigten Zugriffsrechte bestätigen oder ablehnen.</span><b>Was AS Gold danach kann</b><span>Freigegebene E-Mails und Anhänge für Fälle nutzen und von Ihnen freigegebene Antwortschreiben über das verbundene Konto versenden.</span></div><span>{connected.gmail?'✓ Verbunden':configured.google?'Bereit zur Verbindung':'Anbieterfreigabe für AS Gold noch nicht abgeschlossen'}</span><a style={button} href="/api/integrations/google/start?service=gmail">{connected.gmail?'Erneut verbinden':'Gmail verbinden'}</a></article>
    <article style={card}><b>✉ Outlook / Microsoft 365</b><p>Postfach, Anhänge und Antwortentwürfe über Microsoft Graph anbinden.</p><div style={infoBox}><b>Was Sie tun</b><span>„Outlook verbinden“ wählen, sich bei Microsoft anmelden und die gewünschten Berechtigungen bestätigen.</span><b>Was AS Gold danach kann</b><span>Freigegebene Nachrichten und Anhänge übernehmen und von Ihnen bestätigte Antwortschreiben über Outlook/Microsoft 365 versenden.</span></div><span>{connected.outlook?'✓ Verbunden':configured.microsoft?'Bereit zur Verbindung':'Anbieterfreigabe für AS Gold noch nicht abgeschlossen'}</span><a style={button} href="/api/integrations/microsoft/start">{connected.outlook?'Erneut verbinden':'Outlook verbinden'}</a></article>
    <article style={card}><b>☁ Google Drive</b><p>Dokumente und Exporte in einer freigegebenen Google-Drive-Ablage verwenden.</p><div style={infoBox}><b>Was Sie tun</b><span>„Google Drive verbinden“ wählen, sich bei Google anmelden und den Zugriff bestätigen.</span><b>Was AS Gold danach kann</b><span>Nur die für AS Gold freigegebenen Dateien verwenden und Ergebnisse in der vorgesehenen Drive-Ablage speichern.</span></div><span>{connected.drive?'✓ Verbunden':configured.google?'Bereit zur Verbindung':'Anbieterfreigabe für AS Gold noch nicht abgeschlossen'}</span><a style={button} href="/api/integrations/google/start?service=drive">Google Drive verbinden</a></article>
    <article style={card}><b>☁ OneDrive</b><p>Cloud-Dateien über dieselbe Microsoft-Verbindung nutzen und Ergebnisse dort ablegen.</p><div style={infoBox}><b>Was Sie tun</b><span>„OneDrive verbinden“ wählen, sich bei Microsoft anmelden und den gewünschten Dateizugriff freigeben.</span><b>Was AS Gold danach kann</b><span>Freigegebene Dateien für Fälle verwenden und Ergebnisse in Ihrer OneDrive-Ablage speichern.</span></div><span>{connected.onedrive?'✓ Verbunden':configured.microsoft?'Bereit zur Verbindung':'Anbieterfreigabe für AS Gold noch nicht abgeschlossen'}</span><a style={button} href="/api/integrations/microsoft/start">OneDrive verbinden</a></article>
    <article style={card}><b>💾 Eigener Ordner / Gerätespeicher</b><p>Sie entscheiden selbst, wo Exporte und Arbeitsdateien liegen sollen.</p><div style={infoBox}><b>Was Sie tun</b><span>„Eigenen Ordner auswählen“ wählen und einen Ordner auf PC, Mac oder einem unterstützten Gerät freigeben.</span><b>Was AS Gold danach kann</b><span>Ergebnisse in diesem von Ihnen gewählten Speicherort ablegen. Der Zugriff bleibt auf den freigegebenen Ordner begrenzt.</span></div><strong>{folder}</strong><button style={button} onClick={chooseFolder}>Eigenen Ordner auswählen</button>{localNote&&<small>{localNote}</small>}</article>
  ...
  <div className="legalNotice" style={{marginTop:24}}><b>Kontrolle bleibt beim Kunden.</b><p>AS Gold verbindet kein Konto automatisch. OAuth-Verbindungen können erst nach Zustimmung des Kontoinhabers hergestellt werden. Passwörter von Gmail oder Microsoft werden nicht in AS Gold gespeichert; vorgesehen sind kurzlebige Zugriffstoken und verschlüsselte Refresh-Tokens. Eine Verbindung kann später wieder getrennt werden.</p></div>
