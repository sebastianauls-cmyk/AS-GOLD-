...
const notices = {
  ...
  uk:{pwMin:'Пароль має містити щонайменше 12 символів, великі й малі літери, цифру та спеціальний символ.',pwMismatch:'Паролі не збігаються.',registered:'Реєстрація успішна. Підтвердьте електронну адресу, а потім увійдіть.',upgradeReserved:'Підвищення тарифу зарезервовано.',selected:'Обрано',monthOne:'місяць',monthMany:'місяців',chooseFile:'Будь ласка, виберіть файл.',docLimit:'Ваш поточний тариф дозволяє максимум {limit} документів. Оберіть підвищення тарифу.',exportLocked:'Цей формат експорту не входить до вашого поточного тарифу. Оберіть підвищення тарифу.',pptxLoad:'Не вдалося завантажити модуль PowerPoint.'},
...
const dashboardGuide = {
  ...
  uk:{free:{title:'Ласкаво просимо — почніть просто',lead:'Нічого готувати не потрібно. Спочатку завантажте документ, і AS Gold зрозуміло покаже результат.',steps:['1. Завантажте документ','2. Перегляньте результат','3. Вирішіть, чи потрібна більша глибина'],next:'Завантажити перший документ',nextSection:'documents',mode:'Керований старт'},start:{title:'Спочатку впорядкуйте справу',lead:'Створіть клієнта або справу, а потім додайте наявні документи.',steps:['1. Додати клієнта','2. Створити справу','3. Додати документи'],next:'Додати клієнта',nextSection:'clients',mode:'Структурований старт'},klar:{title:'Зробіть прогалини та пріоритети видимими',lead:'Працюйте по справах. Перевіряйте документи, відкриті питання та статус.',steps:['1. Відкрити справу','2. Перевірити документи','3. Визначити пріоритети'],next:'Перевірити справи',nextSection:'cases',mode:'Режим ясності'},analyse:{title:'Оцініть ситуацію, ризики та наступні кроки',lead:'Використовуйте огляд справи як робочий центр.',steps:['1. Обрати справу','2. Перевірити ризики','3. Визначити наступні кроки'],next:'Відкрити аналіз',nextSection:'cases',mode:'Режим аналізу'},komplett:{title:'Опрацьовуйте справу як повний процес',lead:'Клієнти, справи, документи, результати та погодження ведуться в одному процесі.',steps:['1. Перевірити стан','2. Підготувати результати','3. Перевірити погодження'],next:'Відкрити робочі справи',nextSection:'cases',mode:'Режим опрацювання'},business:{title:'Ваш центр керування Business',lead:'Керуйте кількома клієнтами та справами паралельно.',steps:['Керувати клієнтами й справами','Контролювати погодження','Контролювати документи та процеси'],next:'Керувати Business-справами',nextSection:'cases',mode:'Business-керування'}},
  ru:{free:{title:'Добро пожаловать — начните просто',lead:'Ничего готовить не нужно. Сначала загрузите документ, и AS Gold понятно покажет результат.',steps:['1. Загрузите документ','2. Посмотрите результат','3. Решите, нужна ли большая глубина'],next:'Загрузить первый документ',nextSection:'documents',mode:'Пошаговый старт'},start:{title:'Сначала наведите порядок',lead:'Создайте клиента или дело, затем прикрепите документы.',steps:['1. Добавить клиента','2. Создать дело','3. Прикрепить документы'],next:'Добавить клиента',nextSection:'clients',mode:'Структурированный старт'},klar:{title:'Сделайте пробелы и приоритеты видимыми',lead:'Работайте по делам. Проверяйте документы, открытые вопросы и статус.',steps:['1. Открыть дело','2. Проверить документы','3. Расставить приоритеты'],next:'Проверить дела',nextSection:'cases',mode:'Режим ясности'},analyse:{title:'Оцените ситуацию, риски и следующие шаги',lead:'Используйте обзор дела как рабочий центр.',steps:['1. Выбрать дело','2. Проверить риски','3. Определить следующие шаги'],next:'Открыть анализ',nextSection:'cases',mode:'Режим анализа'},komplett:{title:'Ведите дело как полный процесс',lead:'Клиенты, дела, документы, результаты и согласования ведутся в едином процессе.',steps:['1. Проверить состояние','2. Подготовить результаты','3. Проверить согласования'],next:'Открыть рабочие дела',nextSection:'cases',mode:'Режим обработки'},business:{title:'Ваш Business-центр управления',lead:'Управляйте несколькими клиентами и делами параллельно.',steps:['Управлять клиентами и делами','Контролировать согласования','Контролировать документы и процессы'],next:'Управлять Business-делами',nextSection:'cases',mode:'Business-управление'}},
...
const recommendationText = {
  ...
  uk:{title:'Чого ви хочете досягти?',lead:'Оберіть мету. AS Gold рекомендує найдешевший достатній рівень, а не автоматично найдорожчий.',goals:[['overview','Отримати лише перший огляд'],['organize','Впорядкувати документи та справу'],['gaps','Виявити прогалини, суперечності та строки'],['risks','Оцінити ризики та наступні кроки'],['full','Комплексно опрацювати складну справу'],['business','Керувати кількома клієнтами, справами або командою']],recommended:'Наша рекомендація',enough:'Вашого поточного рівня достатньо для цієї мети.',upgradeReason:'Для цієї мети рекомендований рівень додатково дає:',freeNote:'Ви можете залишитися на безкоштовному рівні. Підвищення має сенс лише за потреби більшої глибини чи функцій.',showBenefit:'Показати переваги',chooseGoal:'Оберіть мету'},
...
const transparencyText = {
  ...
  uk:{eyebrow:'Наша обіцянка прозорості',title:'До рішення ви точно знаєте всі умови.',lead:'AS Gold не використовує пасток підписки чи автоматичного продовження. До рішення ви бачите обсяг, строк, ціну та дату завершення.',items:[['Без автоматичного продовження','Новий платний строк починається лише після вашого чіткого вибору.'],['Без прихованих витрат','Перед платним рішенням ви бачите суму сьогодні, звичайну ціну та обраний строк.'],['Чесна рекомендація тарифу','AS Gold рекомендує найдешевший рівень, достатній для вашої мети. Якщо підвищення не потрібне, ми прямо це кажемо.'],['Нагадування перед завершенням','Для 30 днів нагадуємо після 20 днів і ще раз за два дні до кінця. Без активного продовження платний доступ завершується.'],['Чесне підвищення','Під час чинного періоду ви платите лише пропорційну різницю за решту часу.'],['Вирішуєте ви','Без примусової покупки, попередньо вибраного продовження чи тихого продовження.']],registerTitle:'Реєстрація починається безкоштовно.',registerNote:'Безкоштовна реєстрація не запускає платний строк. Витрати виникають лише якщо ви пізніше самі оберете платний рівень і явно підтвердите огляд ціни.'},
...
export default function Home(){
  ...
  async function register(e){
    ...
    if(!/^(?=.*\p{Ll})(?=.*\p{Lu})(?=.*\p{N})(?=.*[^\p{L}\p{N}]).{12,}$/u.test(password)) return setMessage(n.pwMin)
