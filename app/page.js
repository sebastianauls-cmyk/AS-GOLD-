...
import { localeForLanguage, outputLanguageNames, pageTranslations, rtlLanguages, supportedLanguages } from './lib/v30Languages.mjs'
...
function OutputLanguageOptions({language}){
  const labels=outputLanguageNames[language]||outputLanguageNames.de
  return languages.map(item=><option value={item.key} key={item.key}>{labels[item.key]||item.label}</option>)
}
...
export default function Home(){
  ...
  async function doExport(ref,type){
    ...
    try{
      if(type==='docx'){
        ...
        const {default:writeExcelFile}=await import('write-excel-file/browser'); await writeExcelFile(rows).toFile(`${base}.xlsx`)
      } else if(type==='pptx'){
  ...
  function protectedWorkspace(content){
    return <><header className="appTop"><div className="brand"><Logo/><b>AS Gold</b></div><div className="appHeaderTools"><span className="legalChip">{t.legal}</span><select value={language} onChange={e=>setLanguage(e.target.value)} aria-label={t.language}>{languages.map(l=><option value={l.key} key={l.key}>{l.short}</option>)}</select><select value={outputLanguage} onChange={e=>setOutputLanguage(e.target.value)} aria-label={t.outputLanguage}><OutputLanguageOptions language={language}/></select><button className="secondary" onClick={()=>supabase.auth.signOut()}>{a.logout}</button></div></header><main className="appMain">{message&&<div className="note">{message}</div>}{content}</main><LegalFooter language={language}/></>
  }
    ...
    <main>
      <div className="legalMarketBar">
        <div className="wrap">
          ...
          <label>{t.outputLanguage}<select value={outputLanguage} onChange={e=>setOutputLanguage(e.target.value)}><OutputLanguageOptions language={language}/></select></label>
        </div>
      </div>
