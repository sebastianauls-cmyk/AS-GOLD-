...
test('the first action shows an example and translates private and business results independently',async({page},testInfo)=>{
  ...
  await page.getByRole('listbox').getByRole('option',{name:'فارسی',exact:true}).click()
  await expect(page.locator('html')).toHaveAttribute('lang','de')
...
test('interface, output and country-example choices remain independent',async({page},testInfo)=>{
  ...
  await page.getByRole('listbox').getByRole('option',{name:'Polski',exact:true}).click()
  ...
  await page.locator('.publicTop .outputModule button[aria-haspopup="listbox"]').click()
  await page.getByRole('listbox').getByRole('option',{name:'English',exact:true}).click()
  await expect(page.locator('html')).toHaveAttribute('lang','pl')
...
test('every language can be selected through the menu and survives a reload',async({page},testInfo)=>{
  await openPublic(page)
  for(const {key:language,label,rtl} of LANGUAGE_CATALOG){
    await page.locator('.publicTop .interfaceModule button[aria-haspopup="listbox"]').click()
    ...
    await expect(page.locator('html')).toHaveAttribute('lang',language)
    await expect(page.locator('html')).toHaveAttribute('dir',rtl?'rtl':'ltr')
    await expect(page.getByRole('heading',{level:1})).toHaveText(publicExperienceCopy(language).headline)
  ...
  await expect(page.locator('html')).toHaveAttribute('lang','vi')
  await page.reload({waitUntil:'domcontentloaded'})
  await expect(page.locator('.publicTop .outputModule button[aria-haspopup="listbox"]')).toContainText('Polski')
  await expect(page.locator('html')).toHaveAttribute('lang','vi')
  await noHorizontalOverflow(page)
  await capture(page,testInfo,'language-choice-retained')
})

test('language menus remain usable with an open video and preserve country and video choices',async({page},testInfo)=>{
  await openPublic(page)
  const country=page.getByLabel('Herkunftsland',{exact:true})
  const target=page.getByLabel('Land des Falls / Wohnland',{exact:true})
  await expect(country).toBeVisible()
  await expect(target).toBeVisible()
  await country.selectOption('VN')
  await target.selectOption('FR')
  await expect(page.locator('.publicLanguageCountryResults')).toBeVisible()
  await expect(page.locator('.publicLanguageCountryResults')).toContainText('Vietnam ↔ Frankreich')
  const explainer=page.locator('[data-explainer-video-section]')
  await explainer.getByRole('button').click()
  await explainer.getByRole('combobox',{name:'Videosprache',exact:true}).selectOption('pl')
  await page.locator('.publicTop .interfaceModule button[aria-haspopup="listbox"]').click()
  await page.getByRole('listbox').getByRole('option',{name:'English',exact:true}).click()
  await expect(explainer.locator('video')).toBeVisible()
  await expect(explainer.getByRole('combobox',{name:'Video language',exact:true})).toHaveValue('pl')
  await expect(page.getByLabel(publicLanguageCountryCopy('en').home,{exact:true})).toHaveValue('VN')
  await expect(page.locator('#sprachen-rechtsraeume select').nth(1)).toHaveValue('FR')
  const trigger=page.locator('.publicTop .outputModule button[aria-haspopup="listbox"]')
  await trigger.click()
  await page.getByRole('listbox').getByRole('option',{name:'Tiếng Việt',exact:true}).click()
  await expect(trigger).toContainText('Tiếng Việt')
  ...
  await noHorizontalOverflow(page)
  await capture(page,testInfo,'language-menu-over-video')
})
