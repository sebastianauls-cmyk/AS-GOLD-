...
export default function Fixture(){
  ...
  async function finishBackground(){
    ...
    if(reviewFailure.current){savedJob.current={...savedJob.current,status:'failed',error_code:'review_unresolved',issues:[{code:'source',location:'analysis.calculations',reason:'Die Berechnung passt nicht zum angegebenen Original.'},{code:'sender_role',location:'letters',reason:'Das Schreiben setzt eine unbelegte Vollmacht voraus.'}]};return}
      const roadmap={...roadmapTestRecord(),source_documents:current.current.documents.map(({id,title,updated_at})=>({id,title,updated_at})),owner_id:current.current.item.owner_id,source_fingerprint:await roadmapFingerprint(roadmapSource(current.current.item,current.current.documents,[]))}
      roadmap.result.analysis={topics:[{id:'difference',title:'Zusammensetzung der Abzüge',status:'open',conclusion:'Die Differenz ist berechnet. Wofür sie abgezogen wurde, ist noch offen.',conditions:'Die Abrechnung muss den Abzug erklären.',sources:[],step_ids:['anfragen']}],calculations:[{id:'net',title:'Brutto minus netto',topic_ids:['difference'],inputs:[{label:'Brutto',value:'18000',kind:'document',document_id:roadmapTestDocuments[0].id,quote:'Die einmalige Kapitalzahlung beträgt 18.000 EUR brutto und 17.000 EUR netto.'},{label:'Netto',value:'17000',kind:'document',document_id:roadmapTestDocuments[0].id,quote:'Die einmalige Kapitalzahlung beträgt 18.000 EUR brutto und 17.000 EUR netto.'}],expression:'gross-net',result:'1000.00',unit:'EUR',conditions:'Die Art der Abzüge ist ungeklärt.',explanation:'18.000 EUR abzüglich 17.000 EUR ergibt 1.000 EUR.'}],limitations:['Die Berechnungsanlage fehlt.'],research_sources:[]}
