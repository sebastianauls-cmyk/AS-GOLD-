...
The fully simulated 10-topic/24-calculation/25-review fixture uses 35 requests, 11 with scoped sources. Total serialized request bytes fall from 4,858,969 to 4,353,791 (10.4%), including the catalogue overhead. The comparison restores full research in otherwise identical requests; it is not a replay of the historical Sarah run. A negative-control whole-case objection forces a full-context correction and a new complete review before acceptance. These tests prove routing, source retention and workflow gates, not real model correctness, token savings or invoice cost. This measured reduction alone does **not** establish that the prior Sarah run would fit the 600,000-input-token stop; no further paid retry is justified by this fixture alone.
