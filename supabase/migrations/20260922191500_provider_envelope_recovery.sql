-- Unreadable provider response envelopes consume the existing bounded retry.
-- One repeat per interrupted stage, three total, the original expiry and
-- every content/consent/access/quota/result gate remain unchanged. Invalid
-- generated JSON and provider refusals are deliberately not retryable here.
