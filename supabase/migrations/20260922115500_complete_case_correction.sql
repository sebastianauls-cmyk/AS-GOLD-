-- Three substantive candidates, each with at most one input repair.
-- Maximum: planning + 5 research + 6 generation + 3 input repairs +
-- 30 reviews = 45 successful stages; one transport retry fits 46 claims.
-- Fixed 45-minute lifetime and every independent review remain unchanged.
alter table private.case_analysis_work drop constraint case_analysis_work_attempts_check;
alter table private.case_analysis_work add constraint case_analysis_work_attempts_check check(attempts between 0 and 46);
...
begin
  ...
  loop
    ...
    if j.expires_at<=now() or w.attempts>=46 or w.steps>=45 or (j.status='running' and w.failures>=1)
...
begin
  ...
  if j.status<>'queued' or j.expires_at<=now() or w.attempts>=46 or w.steps>=45
...
begin
  ...
  if v_status='completed' then
    ...
    return to_jsonb(j);
  elsif v_status='failed' and p_outcome->>'retry'='true' and w.failures<1 and w.attempts<46 and j.expires_at>now()
