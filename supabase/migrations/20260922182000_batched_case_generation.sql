-- Bound the newly separated calculation-generation stages explicitly.
-- At most 6 research stages + 3 candidates * (1 outline + 4 numeric
-- batches + 1 plan + 10 reviews + 1 shared input repair) = 57 stages.
-- Three existing transport recoveries allow at most 60 claims. No change to
-- the fixed original expiry, daily quota, access checks or publication gates.
alter table private.case_analysis_work drop constraint case_analysis_work_attempts_check;
alter table private.case_analysis_work add constraint case_analysis_work_attempts_check check(attempts between 0 and 60);
alter table private.case_analysis_work drop constraint case_analysis_work_steps_check;
alter table private.case_analysis_work add constraint case_analysis_work_steps_check check(steps between 0 and 57);
...
begin
  ...
  loop
    ...
    if j.expires_at<=now() or w.attempts>=60 or w.steps>=57 or (j.status='running' and (w.failures>=1 or w.transport_retries>=3))
...
begin
  ...
  if j.status<>'queued' or j.expires_at<=now() or w.attempts>=60 or w.steps>=57
...
begin
  ...
  if v_status='completed' then
  ...
  elsif v_status='processing' and w.steps<56 and p_outcome->>'checkpoint' is not null and p_outcome->>'stage' in ('planning','research','generation','review','correction') then
    ...
    return to_jsonb(j);
  elsif v_status='failed' and p_outcome->>'retry'='true' and w.failures<1 and w.transport_retries<3 and w.attempts<60 and j.expires_at>now()
