-- Preserve progress across independent transient failures: one retry per
-- interrupted step, at most three per job. Successful stages reset only the
-- consecutive failure count. The original fixed 45-minute expiry never moves.
alter table private.case_analysis_work add column transport_retries integer not null default 0 check(transport_retries between 0 and 3);
alter table private.case_analysis_work drop constraint case_analysis_work_attempts_check;
alter table private.case_analysis_work add constraint case_analysis_work_attempts_check check(attempts between 0 and 48);

alter table private.case_analysis_work drop constraint case_analysis_work_steps_check;
alter table private.case_analysis_work add constraint case_analysis_work_steps_check check(steps between 0 and 45);
...
begin
  ...
  loop
    ...
    if j.expires_at<=now() or w.attempts>=48 or w.steps>=45 or (j.status='running' and (w.failures>=1 or w.transport_retries>=3))
...
begin
  ...
  if j.status<>'queued' or j.expires_at<=now() or w.attempts>=48 or w.steps>=45
...
begin
  ...
  if v_status='completed' then
    ...
    update private.case_analysis_work set failures=failures+1,transport_retries=transport_retries+1,available_at=now()+interval '30 seconds',lease=null,lease_until=null where job_id=j.id;
