-- One mechanical input repair and one substantive correction are independent.
-- Maximum successful calls: planning + 5 research + 3 initial generation +
-- 4 initial reviews + 2 corrected generation + 4 final reviews = 19.
-- The unchanged 20-claim cap leaves room for the existing ONE transport retry.
-- A measured live run took 20 minutes before content correction was possible.
-- New jobs have a fixed 45-minute lifetime; existing jobs are never extended.
alter table public.case_analysis_jobs alter column expires_at set default now()+interval '45 minutes';

alter table private.case_analysis_work drop constraint case_analysis_work_steps_check;
alter table private.case_analysis_work add constraint case_analysis_work_steps_check check(steps between 0 and 19);
...
begin
  ...
  loop
    ...
    if j.expires_at<=now() or w.attempts>=20 or w.steps>=19 or (j.status='running' and w.failures>=1)
...
begin
  ...
  if j.status<>'queued' or j.expires_at<=now() or w.attempts>=20 or w.steps>=19
...
begin
  ...
  elsif v_status='processing' and w.steps<18 and p_outcome->>'checkpoint' is not null and p_outcome->>'stage' in ('planning','research','generation','review','correction') then
