-- A newly corrected substantive candidate may require its own one input repair.
-- Maximum: planning + 5 research + 4 generation + 2 input repairs +
-- 20 reviews = 32 successful stages; one transport retry fits 33 claims.
-- Fixed 45-minute lifetime and one substantive correction remain unchanged.
alter table private.case_analysis_work drop constraint case_analysis_work_attempts_check;
alter table private.case_analysis_work add constraint case_analysis_work_attempts_check check(attempts between 0 and 33);

alter table private.case_analysis_work drop constraint case_analysis_work_steps_check;
alter table private.case_analysis_work add constraint case_analysis_work_steps_check check(steps between 0 and 32);
...
begin
  ...
  loop
    ...
    if j.expires_at<=now() or w.attempts>=33 or w.steps>=32 or (j.status='running' and w.failures>=1)
...
begin
  ...
  if j.status<>'queued' or j.expires_at<=now() or w.attempts>=33 or w.steps>=32
...
begin
  ...
  if v_status='completed' then
    if jsonb_typeof(p_outcome->'result') is distinct from 'object' or p_outcome->>'workflow_version' is null
      ...
      or (case
        ...
        when p_outcome#>>'{result,analysis,verification,version}' in ('v162','v163') then not private.case_analysis_review_coverage_valid(p_outcome->'result')
        else true end)
  ...
  elsif v_status='processing' and w.steps<31 and p_outcome->>'checkpoint' is not null and p_outcome->>'stage' in ('planning','research','generation','review','correction') then
    ...
    return to_jsonb(j);
  elsif v_status='failed' and p_outcome->>'retry'='true' and w.failures<1 and w.attempts<33 and j.expires_at>now()
