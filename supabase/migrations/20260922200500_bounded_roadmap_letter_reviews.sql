-- Bound every high-effort review: at most 3 topics, 6 calculations,
-- 4 facts/questions, 3 actions or 1 complete letter per request. An overview
-- review retains whole-case completeness and cross-part consistency.
-- Max 25 reviews: 4 topics + 4 calculations + 1 overview + 6 records
-- + 4 action batches + 6 letters. Every assigned part is required by SQL.
-- 6 research + 3 * (4 topics + 1 manifest + 4 numbers + 1 plan + 25 reviews
-- + 1 shared input repair) = 114 stages; existing recoveries allow 117 claims.
-- Original expiry, daily quota, access/consent and recovery budgets unchanged.
alter table private.case_analysis_work drop constraint case_analysis_work_attempts_check;
alter table private.case_analysis_work add constraint case_analysis_work_attempts_check check(attempts between 0 and 117);
alter table private.case_analysis_work drop constraint case_analysis_work_steps_check;
alter table private.case_analysis_work add constraint case_analysis_work_steps_check check(steps between 0 and 114);
...
begin
  ...
  if v_verification->>'version'='v168' then
...
begin
  ...
  if j.status<>'queued' or j.expires_at<=now() or w.attempts>=117 or w.steps>=114
  ...
  if v_status='completed' then
    if jsonb_typeof(p_outcome->'result') is distinct from 'object' or p_outcome->>'workflow_version' is null
      ...
      or (case
        ...
        when p_outcome#>>'{result,analysis,verification,version}' in ('v162','v163','v164','v168') then not private.case_analysis_review_coverage_valid(p_outcome->'result')
        else true end)
