-- Bounded topic generation adds at most four stages per candidate.
-- 6 research + 3 * (4 topic batches + 1 numerical manifest + 4 numerical
-- batches + 1 plan + 10 reviews + 1 shared input repair) = 69 stages.
-- The existing three transport recoveries permit at most 72 claims. The
-- original one-hour expiry, daily quota and every result gate are unchanged.
alter table private.case_analysis_work drop constraint case_analysis_work_attempts_check;
alter table private.case_analysis_work add constraint case_analysis_work_attempts_check check(attempts between 0 and 72);
alter table private.case_analysis_work drop constraint case_analysis_work_steps_check;
alter table private.case_analysis_work add constraint case_analysis_work_steps_check check(steps between 0 and 69);
...
begin
  ...
  loop
    ...
    if j.expires_at<=now() or w.attempts>=72 or w.steps>=69 or (j.status='running' and (w.failures>=1 or w.transport_retries>=3))
...
begin
  ...
  if j.status<>'queued' or j.expires_at<=now() or w.attempts>=72 or w.steps>=69
...
begin
  ...
  if v_status='completed' then
  ...
  elsif v_status='processing' and w.steps<68 and p_outcome->>'checkpoint' is not null and p_outcome->>'stage' in ('planning','research','generation','review','correction') then
    ...
    return to_jsonb(j);
  elsif v_status='failed' and p_outcome->>'retry'='true' and w.failures<1 and w.transport_retries<3 and w.attempts<72 and j.expires_at>now()
