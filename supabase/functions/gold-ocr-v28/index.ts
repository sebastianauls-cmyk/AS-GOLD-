...
Deno.serve(async(req:Request)=>{
  ...
  if(!url||!anon) return reply(req,{error:'Dienst nicht konfiguriert'},503);
  ...
  if(!['synthetic','anonymized'].includes(document.data_classification)) return reply(req,{error:'Im Test sind nur synthetische oder wirksam anonymisierte Daten zulässig'},403);
  ...
  const {data:consumed,error:consumeError}=await client.from('documents').update({ai_processing_allowed:false,updated_at:new Date().toISOString()}).eq('id',documentId).eq('owner_id',user.id).eq('ai_processing_allowed',true).select('id').maybeSingle();
  if(consumeError) return reply(req,{error:'Dokumentfreigabe konnte nicht verbraucht werden'},503);
  if(!consumed) return reply(req,{error:'Dokumentfreigabe wurde bereits verwendet. Bitte bestätigen Sie die Analyse erneut.'},409);
  ...
  await client.from('documents').update({ai_last_processed_at:processedAt,ai_notice_version:PRIVACY_NOTICE_VERSION,ai_provider:'openai'}).eq('id',documentId).eq('owner_id',user.id);
  return reply(req,{status:'completed',message:'Dokument wurde automatisch ausgelesen. Bitte Ergebnis prüfen und bewusst speichern.',output_language:requestedOutputLanguage,suggested_case_id:null,case_match_reason:null,...parsed});
});
