...
export async function reviewModelCandidate({providerKey,candidate,reviewContent,deadline=Date.now()+45000,fetchImpl=fetch,onResponse,beforeRequest,attempt=1,callTimeoutMs=90000,reviewModel='gpt-5.6-luna',reviewFocus='',reviewLocations=null,normalizeReviewIssues=null,previousReview=null,cachePrefixLength=0,cacheSharedPrefixLength=0,cacheKey=null}) {
  ...
  if(reviewLocations){
    if(!Array.isArray(reviewLocations)||!reviewLocations.length||reviewLocations.some(location=>!present(location)))throw new ModelWorkflowError('Ungültige Zuordnung der Prüffelder.',500,'review_invalid')
    request.text.format.schema=structuredClone(REVIEW_SCHEMA)
    request.text.format.schema.properties.issues.items.properties.location.enum=reviewLocations
  }
  if(cachePrefixLength){
    ...
    if(!Number.isInteger(cacheSharedPrefixLength)||cacheSharedPrefixLength<0||cacheSharedPrefixLength>=cachePrefixLength||cacheSharedPrefixLength&&reviewContent[cacheSharedPrefixLength-1]?.type!=='input_text')throw new ModelWorkflowError('Ungültige Konfiguration der Fallprüfung.',500,'review_cache_invalid')
    // Complete-case reviews share originals and may share fetched texts. Keep their
    ...
    if(cacheSharedPrefixLength)prefix[cacheSharedPrefixLength-1]={...prefix[cacheSharedPrefixLength-1],prompt_cache_breakpoint:{mode:'explicit'}}
  ...
  const parsedIssues=validateQualityReview(review.parsed)
  const issues=normalizeReviewIssues?normalizeReviewIssues(parsedIssues):parsedIssues
  if(reviewLocations&&issues.some(issue=>!reviewLocations.includes(issue.location)))throw new ModelWorkflowError('Die Gegenprüfung hat einen nicht zugewiesenen Abschnitt genannt.',502,'review_invalid',issues)
