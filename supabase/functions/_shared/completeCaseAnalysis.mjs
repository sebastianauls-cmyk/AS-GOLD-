...
import { searchRetrievedSources, researchSourceCandidates, retrieveOfficialEvidence, primarySourceCatalogue, loadPrimarySources } from './verifiedResearch.mjs'
...
export async function advanceCompleteAnalysis({providerKey,source,style,outputLanguage,referenceLanguage,baseRequest,baseReviewContent,draftLetters=true,state=null,fetchImpl=fetch,onResponse}){
  ...
  if(['research','research_recovery'].includes(current.stage)){
    ...
    const retrieved=await retrieveOfficialEvidence(urls.map(url=>candidates.get(url)).filter(Boolean),countryScope.domains,{fetchImpl,maxSources:20,maxChars:22000,snapshotRecords})
    const research=[...new Map([...(current.research||[]),...retrieved.values()].map(item=>[item.url,item])).values()].slice(0,28)
