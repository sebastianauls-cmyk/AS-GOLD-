...
export async function advanceCompleteAnalysis({providerKey,source,style,outputLanguage,referenceLanguage,baseRequest,baseReviewContent,draftLetters=true,state=null,fetchImpl=fetch,onResponse,beforeRequest}){
  ...
  if(!current.modelState||current.modelState.stage==='generation'){
    ...
    partRequest.input[0]={role:'user',content:[
      ...
      // The complete prior candidate and its feedback repeat across a correction
      // round. Put them before changing assignments/partial work so they can be
      // reused too. New feedback or an input repair changes this exact prefix;
      // the earlier evidence boundaries remain independently reusable.
      ...(correction?[{type:'input_text',text:JSON.stringify({correction}),prompt_cache_breakpoint:{mode:'explicit'}}]:[]),
