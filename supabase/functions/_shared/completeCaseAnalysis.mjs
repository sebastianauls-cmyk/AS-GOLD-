...
export const COMPLETE_ANALYSIS_VERSION='v161'
const REVIEW_SCOPES=['analysis','calculations','roadmap','letters']
...
Apply each fetched rule together with its exceptions and its scope. A new benefit's start year alone may not settle a rule that also depends on predecessor benefits or earlier periods. State missing predicates precisely as scenario conditions and identify how to check them. Distinguish mandatory filings from optional refund requests; check relevant filing triggers rather than calling every unresolved return optional. If known facts meet a rule's main conditions and only an exception remains open, explain that specific exception and the action required if it does not apply; do not leave the entire duty generically unresolved.
...
export async function advanceCompleteAnalysis({providerKey,source,style,outputLanguage,referenceLanguage,baseRequest,baseReviewContent,draftLetters=true,state=null,fetchImpl=fetch,onResponse}){
  ...
  if(!current.modelState||current.modelState.stage==='generation'){
    ...
    const correction=attempt===2||inputRepair?{previous_candidate:inputRepair?{...current.modelState?.previous,analysis:inputRepair.previous}:current.modelState.previous,issues:[...(attempt===2?current.modelState.feedback:[]),...(inputRepair?.feedback||[])],instruction:'Resolve each valid defect against the originals. Preserve unaffected supported content and all needed letters. Do not invent facts to satisfy feedback.'}:null
    ...
    if(!writingPlan){
      ...
      try{draftAnalysis=validateAnalysisContent(resolveQuotationIds(draftAnalysis,quotes,{pathPrefix:'analysis'}),source,{scope:current.scope,research:current.research})}
        ...
        // One input repair across the entire job, independent of the one
        // substantive correction. Neither repair skips any of the four reviews.
        if(current.inputRepair?.used)throw new ModelWorkflowError('Die Berechnungen konnten ihren Originalbelegen noch nicht sicher zugeordnet werden. Es wurde kein neues Ergebnis gespeichert.',422,'source_unresolved',feedback)
        return {status:'processing',state:{...current,draftAnalysis:null,draftFeedback:[],inputRepair:{used:true,previous:generated.parsed,feedback},modelState:current.modelState||{stage:'generation',attempt:1,previous:null,feedback:[]}}}
      }
      return {status:'processing',state:{...current,draftAnalysis,draftFeedback,inputRepair:current.inputRepair?.used?{used:true}:null,analysis_response_id:generated.response_id,modelState:current.modelState||{stage:'generation',attempt:1,feedback:[],previous:null}}}
    }
  ...
  if(!Number.isInteger(partIndex)||partIndex<0||partIndex>=REVIEW_SCOPES.length||(current.reviewIds||[]).length!==partIndex)throw new ModelWorkflowError('Ungültiger Prüfabschnitt.',409)
  ...
  // The former final call reviewed the entire roadmap AND all letters. A live
  // case exceeded its request timeout twice. Keep the same full evidence and
  // high-effort review, but checkpoint these two scopes independently.
  const related={topics:analysis?.topics?.map(({id,title,conclusion,conditions,step_ids})=>({id,title,conclusion,conditions,step_ids})),calculations:analysis?.calculations?.map(({id,title,result,unit,conditions,topic_ids})=>({id,title,result,unit,conditions,topic_ids})),steps:partIndex===3?plan.steps:plan.steps?.map(({id,action,done_when})=>({id,action,done_when})),letters:letters?.map(({id,recipient,subject,document_ids})=>({id,recipient,subject,document_ids}))}
  const part=partIndex===0?{analysis:{topics:analysis?.topics,limitations:analysis?.limitations}}:partIndex===1?{analysis:{calculations:analysis?.calculations}}:partIndex===2?roadmap:{letters}
  const focus=['Review all substantive conclusions, legal applicability, source support and completeness of the case issues. Use related_output to check numerical coverage and linked practical actions. Flag omitted useful conditional financial scenarios when supported; do not re-audit the full calculation input mechanics or letter wording here.','Review every calculation: literal-source number, role, unit, exact arithmetic, time period, assumption, legal applicability and narrative numerical consistency. Use related_output as context. Every relevant calculation must be checked; do not rewrite legal conclusions or letters in this part.','Review every customer-facing roadmap field, original-document fact, question, step, deadline and dependency. Compare them with the related checked analysis and numerical results. Letter identities and purposes are supplied as context; their complete wording and translations receive the separate mandatory letters review. Do not repeat the full legal-source and arithmetic-input audit.','Review every formal letter and its complete customer translation against all originals, fetched sources, related conclusions, numerical results and roadmap steps. Check sender role, recipient, purpose, source fidelity, amounts, dates, language, translation completeness and consistency with the proposed actions. Also flag a missing necessary grounded letter; an empty letters array does not waive this review. Roadmap steps are context, not fields to re-audit here.'][partIndex]
  const review=await reviewModelCandidate({providerKey,candidate:part,reviewContent:[...reviewContent,{type:'input_text',text:JSON.stringify({related_output:related})}],reviewModel:model,reviewFocus:focus+' All original documents remain supplied. Four independent scoped reviews are mandatory before acceptance. Fields assigned to another part are context, not missing candidate fields. Return precise original full-result field locations for any concrete material defect.',deadline:Date.now()+140000,callTimeoutMs:135000,fetchImpl,onResponse:event=>onResponse?.({...event,review_part:partIndex+1,review_scope:REVIEW_SCOPES[partIndex]}),attempt})
  ...
  if(partIndex<REVIEW_SCOPES.length-1)return {status:'processing',state:{...current,reviewIndex:partIndex+1,reviewFeedback:feedback,reviewIds,modelState:{...current.modelState,candidate,validationContext,structuralFeedback}}}
  if(feedback.length){
