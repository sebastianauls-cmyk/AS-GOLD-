...
function untrustedCase(caseRow:any,documents:any[]){
  return JSON.stringify({
    ...
    documents:documents.map(document=>({title:text(document.title,240),type:text(document.document_type,120),data_classification:document.data_classification,original_text:typeof document.extracted_text==='string'?document.extracted_text:'',previous_summary_unverified:text(document.analysis_summary,1600)}))
  });
}
