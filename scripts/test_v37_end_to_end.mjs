...
const documentFileIntake=fs.readFileSync('app/modules/documents/DocumentFileIntake.js','utf8')
const voiceContextInput=fs.readFileSync('app/modules/documents/VoiceContextInput.js','utf8')
...
mustContain(firstActionLegacy,"./FirstAction",'V37 first-action compatibility-only bridge')
...
mustContain(firstActionCompatibility,"../modules/public/V37FirstAction",'first-action external compatibility re-export')
...
mustContain(publicLanding,'href="#beispiel"','direct product example before registration')
...
mustContain(documentsSurface,'DocumentFileIntake','modular document intake mount')
mustContain(documentsSurface,'VoiceContextInput','modular microphone context mount')
mustContain(documentFileIntake,'name="file"','current document upload input')
mustContain(documentFileIntake,'name="source_language"','document source language input')
mustContain(documentFileIntake,'name="intake_quality"','document intake quality input')
mustContain(voiceContextInput,'name="voice_context"','confirmed spoken context input')
mustContain(voiceContextInput,'SpeechRecognition','browser speech recognition boundary')
...
mustContain(workspaceRepository,"traffic_light:normalizeTrafficLight(payload.traffic_light)",'validated case traffic light with safe yellow fallback')
...
console.log('V98 end-to-end regression checks passed against modular document intake and current service boundaries')
