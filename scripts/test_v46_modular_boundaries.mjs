...
const required=[
  ...
  'app/modules/documents/documentWorkflow.js',
  'app/modules/documents/exportWorkflow.js',
  'app/modules/services/documentAnalysis.js',
  'app/modules/cases/caseWorkflow.js',
  'app/modules/cases/approvalWorkflow.js',
  'app/modules/auth/workspaceAuthWorkflow.js',
  'app/modules/pricing/pricingWorkflow.js',
  'app/modules/compliance/accountWorkflow.js',
  'app/modules/public/PublicLanding.js',
  'app/modules/workspace/ProtectedWorkspaceShell.js',
  'supabase/functions/gold-document-analysis/index.ts'
]
...
const documentService=read('app/modules/services/documentAnalysis.js')
const exportWorkflow=read('app/modules/documents/exportWorkflow.js')
const edge=read('supabase/functions/gold-document-analysis/index.ts')
...
assert.match(active,/useLanguagePreferences/)
assert.match(active,/DocumentsSurface/)
assert.match(active,/PublicLanding/)
assert.doesNotMatch(active,/from '@supabase\/supabase-js'/)
assert.doesNotMatch(active,/const supabase\s*=\s*createClient/)

assert.match(registry,/LANGUAGE_CATALOG/,'central language registry must own the catalog')
assert.match(registry,/supportedLanguages/)
assert.match(registry,/localeForLanguage/)
assert.match(registry,/pageTranslations/)
assert.match(languageFacade,/languageRegistry\.mjs/,'legacy language facade must delegate to registry')
assert.doesNotMatch(languageFacade,/export const LANGUAGE_CATALOG|LANGUAGE_CATALOG\s*=\s*Object\.freeze/,'legacy language facade must not define a second catalog')
...
assert.match(documentWorkflow,/dataClassification=form\.elements\.data_classification\?\.value\|\|'personal'/)
assert.doesNotMatch(documentWorkflow,/\['synthetic','anonymized'\]\.includes/)
assert.match(documentService,/gold-document-analysis/)
assert.doesNotMatch(documentService,/gold-ocr-v28/)
assert.match(edge,/OUTPUT_LANGUAGES/)
assert.match(exportWorkflow,/createWorkspaceExportArtifact/)
assert.match(exportWorkflow,/canExport/)

const layout=read('app/layout.js')
assert.doesNotMatch(layout,/MutationObserver|OutputLanguageBridge|V44LanguageOrder/,'global layout must not patch product behavior')
