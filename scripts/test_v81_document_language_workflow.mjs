...
assert.deepEqual(DOCUMENT_LANGUAGE_WORKFLOW_STEPS,['original','translation','explanation','next_step','reference_copy','customer_copy'])
...
assert.equal(DOCUMENT_LANGUAGE_WORKFLOW_FIELDS.reference_copy,'reference_copy')
...
assert.match(edge,/REFERENZSPRACHE|versandfertiges Antwortschreiben/i)
assert.match(edge,/Referenzsprache, Kundensprache und Land müssen unabhängig/)
