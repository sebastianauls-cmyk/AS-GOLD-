...
assert.match(languageFacade,/languageRegistry\.mjs/,'remaining V36 facade must delegate only to central registry')
assert.doesNotMatch(languageFacade,/LANGUAGE_CATALOG\s*=|export const supportedLanguages|pageTranslations\s*=/,'remaining V36 facade must not own language data')
assert.match(languageRegistry,/LANGUAGE_CATALOG/,'central language registry must own the language catalog')
console.log('V80 language-order guard passed: WorkspaceController uses the central language registry; remaining V36 facade is compatibility-only.')
