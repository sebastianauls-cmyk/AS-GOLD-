...
assert.equal(APP_VERSION,'V79','release version must be changed only in the central release module')
...
for(const file of walk('app')){
  ...
  assert.doesNotMatch(source,/AS Gold|AS GOLD|AS%20Gold/,`${file}: obsolete public brand must not return`)
}
