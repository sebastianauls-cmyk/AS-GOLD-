...
import {componentTranslations} from '../app/modules/language/componentTranslations.mjs'
import {localeForLanguage,outputLanguageNames,pageTranslations,supportedLanguages} from '../app/modules/language/languageRegistry.mjs'
import {normalizeOutputLanguage,outputLanguageLabels,withOutputLanguage} from '../app/modules/language/outputLanguage.js'
import {legalPageIds,legalShellCopy,legalTranslations} from '../app/modules/compliance/legalTranslations.mjs'
...
const active=[
  'app/modules/language/LanguageSwitcher.js','app/modules/language/ExplainerVideoDialog.js','app/modules/public/ExplainerVideo.js','app/modules/public/ProblemNavigator.js','app/modules/public/FirstAction.js','app/modules/public/InstallAppButton.js'
]
...
const landing=fs.readFileSync('app/modules/public/PublicLanding.js','utf8');assert.match(landing,/<LegalFooter language=\{language\}/);assert.match(landing,/outputLanguage=\{outputLanguage\}/);assert.match(fs.readFileSync('app/modules/language/LanguageSwitcher.js','utf8'),/supportedLanguages\.find/);assert.match(landing,/\bvi:\{/);assert.match(landing,/PDF.*Word/);assert.doesNotMatch(landing,/href=\{language==='de'\?'\/testen'/)
const languageModules=fs.readFileSync('app/modules/public/PublicLanguageModules.js','utf8');assert.match(languageModules,/vi:'Chào mừng bạn đến với ASH Workspace Gold/)
