...
const languageModules=fs.readFileSync('app/modules/public/PublicLanguageModules.js','utf8');assert.match(languageModules,/vi:'Chào mừng bạn đến với AS Gold/)
const heroCopy=fs.readFileSync('app/modules/public/HeroCopyEnhancer.js','utf8');assert.match(heroCopy,/vi:\{title:'AS Gold – sự rõ ràng/);assert.match(heroCopy,/AS Gold đặc biệt hữu ích cho ai/)
const problem=fs.readFileSync('app/modules/public/ProblemNavigator.js','utf8');for(const token of ['Thử miễn phí với 3 tài liệu','Cách nhập vấn đề:','Vấn đề là gì?','rủi ro','thời hạn'])assert.ok(problem.includes(token),`missing Vietnamese public navigator text: ${token}`)
