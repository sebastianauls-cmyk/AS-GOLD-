...
for(const lang of expectedLanguages){const rec=prioritizeNextStep({language:lang,missing:true});assert.equal(rec.kind,'missing',`${lang}: Prioritätslogik weicht ab`);assert.ok(rec.action.length>4,`${lang}: Übersetzung der Hauptempfehlung fehlt`);const urgent=prioritizeNextStep({language:lang,missing:true,deadlineStatus:'immediate'});assert.equal(urgent.kind,'deadline',`${lang}: akute Frist muss Vorrang haben`);assert.ok(urgent.action.length>8,`${lang}: kombinierte Frist-/Unterlagenempfehlung fehlt`)}
...
assert.doesNotMatch(layout,/V38DeadlineCardEnhancer/);assert.match(directCases,/DeadlineWarningCard/)
for(const component of ['V38AssessmentExplainability','V38PrimaryNextStep']) assert.doesNotMatch(layout,new RegExp(component))
assert.match(directCases,/AssessmentExplainability/);assert.match(directCases,/PrimaryNextStepCard/)
console.log('V80 synthetic full-flow simulation passed through WorkspaceController and version-neutral intelligence/export module boundaries.')
