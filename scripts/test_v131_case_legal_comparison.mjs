...
const officeExports=read('app/modules/services/officeExports.js')
...
assert.match(exportService,/createTextPdf/,'All PDF exports must use the shared text renderer')
assert.match(fs.readFileSync('app/modules/services/textPdf.mjs','utf8'),/pdf\.circle\(/,'PDF traffic lights must be drawn as real vector circles')
...
assert.match(officeExports,/TRAFFIC_COLORS/,'Excel and PowerPoint traffic colors must be defined explicitly')
assert.match(officeExports,/worksheetInlineString/,'Excel must render traffic markers as rich colored runs')
assert.match(officeExports,/slideRunProperties/,'PowerPoint must render traffic markers as independently colored runs')
...
const wordZip=await JSZip.loadAsync(await wordArtifact.blob.arrayBuffer())
...
const xlsxArtifact=await createWorkspaceExportArtifact({ref:{kind:'case',item:syntheticCase},type:'xlsx',data:syntheticData,copy:exportCopy,outputLanguage:'de'})
assert.equal(xlsxArtifact.filename,'Synthetischer_Abschlussfall.xlsx')
assert.ok(xlsxArtifact.blob.size>500,'synthetic Excel export must produce a non-empty workbook')
const xlsxZip=await JSZip.loadAsync(await xlsxArtifact.blob.arrayBuffer())
const sheetXml=await xlsxZip.file('xl/worksheets/sheet1.xml').async('string')
assert.match(sheetXml,/FF2F855A/i,'Excel output must contain a real green traffic-light color')
assert.match(sheetXml,/FFD69E2E/i,'Excel output must contain a real yellow traffic-light color')
assert.match(sheetXml,/FFC53030/i,'Excel output must contain a real red traffic-light color')
assert.match(sheetXml,/●/,'Excel output must contain visible traffic-light dots')
assert.match(sheetXml,/Synthetischer Abschlussfall/)
assert.match(sheetXml,/Antwortentwurf/)

const pptxArtifact=await createWorkspaceExportArtifact({ref:{kind:'case',item:syntheticCase},type:'pptx',data:syntheticData,copy:exportCopy,outputLanguage:'de'})
assert.equal(pptxArtifact.filename,'Synthetischer_Abschlussfall.pptx')
assert.ok(pptxArtifact.blob.size>500,'synthetic PowerPoint export must produce a non-empty presentation')
const pptxZip=await JSZip.loadAsync(await pptxArtifact.blob.arrayBuffer())
const slideFiles=Object.keys(pptxZip.files).filter(name=>/^ppt\/slides\/slide\d+\.xml$/.test(name))
assert.ok(slideFiles.length>=2,'PowerPoint export must contain detail slides')
const slideXml=await Promise.all(slideFiles.map(name=>pptxZip.file(name).async('string'))).then(parts=>parts.join('\n'))
assert.match(slideXml,/2F855A/i,'PowerPoint output must contain a real green traffic-light color')
assert.match(slideXml,/D69E2E/i,'PowerPoint output must contain a real yellow traffic-light color')
assert.match(slideXml,/C53030/i,'PowerPoint output must contain a real red traffic-light color')
assert.match(slideXml,/●/,'PowerPoint output must contain visible traffic-light dots')
assert.match(slideXml,/Synthetischer Abschlussfall/)
assert.match(slideXml,/Antwortentwurf/)

console.log('V131 final synthetic output guard passed: document analysis, colored traffic lights, deadline, next step, bilingual letter, approval plus Word, Excel and PowerPoint exports remain complete.')
