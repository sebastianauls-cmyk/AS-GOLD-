...
const documentConfig=document.slice(document.indexOf('\n',document.indexOf('// MODEL_WORKFLOW_START'))+1,document.indexOf('// MODEL_CONFIGURATION_END'))
if(!documentConfig.includes('const validate=')||documentConfig.includes('const binding='))throw new Error('Document model configuration boundary is missing')
...
const imports=`import { advanceCompleteAnalysis } from './completeCaseAnalysis.mjs';\nimport { CASE_EVIDENCE_RULES } from './caseEvidenceRules.mjs';\nimport { ROADMAP_SCHEMA,validateRoadmapResult,splitVerbatimRoadmapEvidence,roadmapModelSource } from './customerRoadmap.mjs';\nimport { originalPlainText,finalizeDocumentResult,runReviewedModel,advanceReviewedModel } from './modelQuality.mjs';\n`
...
const source=imports+base64+'\n'+languageLine+`\nexport async function evaluateDocument({providerKey,bytes,fileMime='text/plain',filePath='synthetic.txt',requestedOutputLanguage='de',requestedReferenceLanguage='de',outputLanguageName='Deutsch',referenceLanguageName='Deutsch',countryContextName='Deutschland / deutscher Rechtsraum',voiceContext=null,voiceLanguage=null},onResponse){\n${documentConfig}\nreturn runReviewedModel({providerKey,request,reviewContent,validate,onResponse});\n}\nexport async function evaluateDocumentStage({providerKey,bytes,fileMime='text/plain',filePath='synthetic.txt',requestedOutputLanguage='de',requestedReferenceLanguage='de',outputLanguageName='Deutsch',referenceLanguageName='Deutsch',countryContextName='Deutschland / deutscher Rechtsraum',voiceContext=null,voiceLanguage=null,state=null},onResponse){\n${documentConfig}\nreturn advanceReviewedModel({providerKey,request,reviewContent,validate,state,onResponse});\n}\nexport async function evaluateRoadmap({providerKey,source,style,permissions={full_analysis:true,draft_letters:true},outputLanguage='de',referenceLanguage='de'},onResponse){\n${roadmapConfig}\nreturn runReviewedModel({providerKey,request,reviewContent,validate,onResponse});\n}\nexport async function evaluateRoadmapStage({providerKey,source,style,permissions={full_analysis:true,draft_letters:true},outputLanguage='de',referenceLanguage='de',state=null,complete=false},onResponse){\n${roadmapConfig}\nif(complete)return advanceCompleteAnalysis({providerKey,source,style,outputLanguage,referenceLanguage,baseRequest:request,baseReviewContent:reviewContent,draftLetters:permissions.draft_letters===true,state,onResponse});\nreturn advanceReviewedModel({providerKey,request,reviewContent,validate,state,onResponse});\n}\n`
...
for(const file of ['caseEvidenceRules.mjs','customerRoadmap.mjs','modelQuality.mjs','verifiedResearch.mjs','completeCaseAnalysis.mjs','checkedCalculations.mjs','researchCountries.mjs','quotationIndex.mjs','primarySourceCache.mjs'])fs.copyFileSync('supabase/functions/_shared/'+file,path.join(destination,file))
