...
for(const language of expectedLanguages){
  ...
  assert.match(
    ...
    `missing female fallback for ${language}`
  )
  assert.match(
    maleRemoteBlock,
    new RegExp(`\\b${language}:'https://(?:resource2|files2)\\.heygen\\.ai/[^']+\\.mp4'`),
    `missing male video for ${language}`
  )
}
...
assert.doesNotMatch(
  ...
  /[?&](?:Expires|Signature|Key-Pair-Id)=/i,
  'video catalogs must not use expiring signed query parameters'
)
