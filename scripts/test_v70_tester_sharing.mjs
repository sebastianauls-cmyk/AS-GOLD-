...
assert.match(share,/\bvi:/);assert.match(adapter,/modules\/tester\/TesterShareButton/);assert.match(guide,/APP_VERSION/);assert.match(guide,/AS Workspace Gold sicher ausprobieren/);assert.match(guide,/<TesterShareButton\/>/);assert.match(gate,/Vollständig kostenlosen Testzugang persönlich anfordern/);assert.match(gate,/E-Mail mit Ihrem Namen und der E-Mail-Adresse/);assert.match(page,/PromoTesterGate/);assert.doesNotMatch(page,/TesterGuide|TesterPaused|TesterShareButton/);assert.doesNotMatch(gate,/TesterGuide|\/api\/tester\/promo|sessionStorage/)
...
console.log(`${APP_VERSION} tester-sharing guard passed: legacy sharing support remains isolated, while the public tester route only offers the personal approval request path.`)
