...
assert.match(share,/\bvi:/);assert.match(adapter,/modules\/tester\/TesterShareButton/);assert.match(guide,/AS Gold V74 sicher ausprobieren/);assert.match(guide,/<TesterShareButton\/>/);assert.match(page,/TesterGuide/);assert.doesNotMatch(page,/TesterPaused/)
console.log('V74 tester-sharing release guard passed: native sharing, WhatsApp and clipboard fallback are active through the modular tester route in all eleven languages.')
