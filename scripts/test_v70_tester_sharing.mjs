...
assert.match(share,/\bvi:/);assert.match(adapter,/modules\/tester\/TesterShareButton/);assert.match(guide,/APP_VERSION/);assert.match(guide,/AS Workspace Gold sicher ausprobieren/);assert.match(guide,/<TesterShareButton\/>/);assert.match(gate,/Promo-Code persönlich anfordern/);assert.match(gate,/persönlich per WhatsApp oder E-Mail/);assert.match(page,/PromoTesterGate/);assert.doesNotMatch(page,/TesterGuide|TesterPaused|TesterShareButton/);assert.doesNotMatch(gate,/TesterGuide|\/api\/tester\/promo|sessionStorage/)
