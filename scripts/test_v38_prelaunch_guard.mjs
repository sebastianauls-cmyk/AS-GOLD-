...
const testing=read('app/testen/page.js')+'\n'+read('app/modules/tester/PromoTesterGate.js')
...
assert.equal(packageJson.dependencies?.stripe,undefined,'Der abgelöste Stripe-SDK darf nicht mehr im Produktionspaket liegen')
...
const paymentServer=read('app/modules/payments/sumupServer.js')
assert.match(paymentConfig,/SUMUP_PAYMENT_MODE_SANDBOX/)
assert.match(paymentConfig,/liveLocked/)
assert.match(paymentServer,/merchant\.sandbox!==true/)
assert.match(paymentWebhook,/reconcileSumupCheckout/)
assert.match(paymentWebhook,/sumup_checkout_id/)
...
assert.match(testing,/PromoTesterGate/)
assert.match(testing,/robots:\{index:false,follow:false\}/)
assert.match(testing,/Vollständig kostenlosen Testzugang persönlich anfordern/)
assert.match(testing,/senden Sie dem Anbieter zunächst eine E-Mail/i)
assert.match(testing,/ausdrücklich zustimmt/)
assert.match(testing,/an die angegebene Login-E-Mail gebunden/)
assert.match(testing,/keine Kosten und keine automatische Verlängerung/i)
assert.doesNotMatch(testing,/\/api\/tester\/promo|sessionStorage|aspromo2026/,'tester request page must not contain an automatic promo verifier or fixed code')
...
assert.match(googleStart,/authorization\.startsWith\('Bearer '\)/)
assert.match(googleStart,/sealIntegrationToken\(\{owner_id:user\.id/)
assert.match(googleCallback,/openIntegrationToken/)
assert.match(googleCallback,/state\.provider!=='google'/)
assert.match(googleCallback,/sealIntegrationToken/)
assert.match(microsoftStart,/randomUUID/)
assert.match(microsoftStart,/authorization\.startsWith\('Bearer '\)/)
assert.match(microsoftStart,/sealIntegrationToken\(\{owner_id:user\.id/)
assert.match(microsoftCallback,/openIntegrationToken/)
assert.match(microsoftCallback,/state\.provider!=='microsoft'/)
...
console.log('V131 pre-launch guard passed: tester access requires personal provider approval; payment, privacy and security boundaries remain locked.')
