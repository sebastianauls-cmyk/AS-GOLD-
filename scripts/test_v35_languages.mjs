...
assert.match(videoDialogSource,/ro:'Videoclip explicativ'/)
assert.match(videoDialogSource,/bg:'Обяснително видео'/)
assert.match(videoDialogSource,/ro:'Închide'/)
assert.match(videoDialogSource,/bg:'Затвори'/)
assert.match(switcherSource,/publicPicker\?label:active\.label/)
assert.match(switcherSource,/className="flagLanguageClose"/,'language menu must expose one explicit close/back control')
assert.equal((switcherSource.match(/className="flagLanguageClose"/g)||[]).length,1,'language menu must not duplicate the back control')
...
for(const language of expectedLanguages){
  ...
  assert.match(videoDialogSource,new RegExp(`\\/videos\\/as-gold-v35-${language}\\.mp4`))
}

console.log('V35 language guard: 10 app languages, RO/BG catalogs, SVG flags, modular navigation and local videos verified.')
