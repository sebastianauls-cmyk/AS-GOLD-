...
for(const text of ['APP_VERSION','showRelease','localizable','Testerbetrieb','AS Workspace Gold sicher ausprobieren','Tester-Link weiterleiten','Genau ein Sprachmenü und genau ein Zurück-Element im geöffneten Sprachmenü','Zurück schließt das Sprachmenü zuverlässig, auch auf Mobilgeräten','Oberflächensprache zuerst und getrennte Ausgabesprache danach','Klare Navigation ohne offensichtliche Sackgassen','allen 11 App-Sprachen einschließlich Vietnamesisch','Musterdatei herunterladen','Die Bezahlfunktion bleibt deaktiviert.']){if(!page.includes(text)) throw new Error('Final tester guide missing: '+text)}
if(APP_VERSION!=='V79') throw new Error('Unexpected central app version: '+APP_VERSION)
