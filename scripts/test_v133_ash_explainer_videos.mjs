...
assert.ok(APP_RELEASE.number>=134)
...
for(const presenter of ['female','male']){
  for(const language of expectedLanguages){
    ...
    if(language==='de'){
      assert.equal(video.src,`/videos/ash-workspace-gold-v134-${presenter}-de.mp4`)
    ...
    const captionVersion=language==='de'?134:133
    assert.equal(video.captions,`/captions/ash-workspace-gold-v${captionVersion}-${presenter}-${language}.vtt`)
...
console.log('V134 ASH video guard passed: 22 current videos, 22 local subtitle tracks, corrected German pronunciation and no legacy AS Gold source.')
