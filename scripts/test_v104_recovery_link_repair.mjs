...
assert.ok(Number(APP_VERSION.replace('V',''))>=104)
...
assert.match(result.url,/^https:\/\/app-gold-workspace\.vercel\.app\/\?release=V\d+#access_token=/)
...
assert.match(page,/inspectRecoveryLink\(rawLink\)/)
