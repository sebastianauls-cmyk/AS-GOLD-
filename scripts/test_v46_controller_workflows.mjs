...
const testerPage=read('app/testen/page.js');const promoGate=read('app/modules/tester/PromoTesterGate.js');const testerGuide=read('app/modules/tester/TesterGuide.js');assert.match(testerPage,/PromoTesterGate/);assert.doesNotMatch(testerPage,/TesterGuide|TesterPaused/);assert.match(promoGate,/TesterGuide/);assert.match(promoGate,/\/api\/tester\/promo/);assert.match(testerGuide,/APP_VERSION/)
console.log('V131 controller workflow guard passed: WorkspaceController stays modular and tester content is reachable only behind the promo-code gate.')
