...
need(publicHeader,"onScreenChange('login')",'login path in public header')
need(caseSurfaces,'backOverview','cases/clients overview navigation')
need(page,'onBack={()=>setSelectedCase(null)}','case-detail back navigation')
need(caseWorkspace,'export function CaseDetail','case detail module ownership')
need(caseSurfaces,'backClients','client-detail back navigation')
need(authSurface,'backExplanation','back navigation in auth module')
need(documentWorkflow,'functionErrorMessage','analysis error normalization')
need(documentWorkflow,'configuration_required','analysis configuration error')
need(caseSurfaces,'emptyState','client empty-state presentation')
need(documentsSurface,'disabled={uploading}','upload busy state')
need(caseSurfaces,'required','required-field handling')
need(caseWorkflow,'createAssessment','traffic-light assessment workflow')
need(approvalWorkflow,'prepareDocumentApproval','prepare approval workflow')
need(page,'ApprovalDetail','approval detail route')
need(exportWorkflow,'doExport','export workflow')
need(page,'createExportWorkflowActions','export workflow wiring')
