...
assert.match(contract.approvalGate,/owner|explicit approval/i)
...
assert.throws(()=>implementationTaskFromProposal({status:'pending'}),/owner notice|explicit approval/i)
const approved={id:'x',status:'approved',owner_notified_at:new Date().toISOString(),owner_acknowledged_at:new Date().toISOString(),approved_at:new Date().toISOString(),monitor_kind:'legal_monitor',title:'Test',recommendation:'Change',implementation_scope:['module'],source_urls:['https://example.com']}
assert.equal(implementationTaskFromProposal(approved).production_deploy_allowed,false)
...
console.log('V83 continuous-improvement architecture guard passed: owner-first notice and explicit approval remain mandatory before implementation.')
