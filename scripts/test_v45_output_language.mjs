...
for(const field of ['extracted_text','document_translation','summary','next_step','reference_copy','customer_copy']){
...
assert.match(fn,/Referenzsprache des Antwortschreibens/)
assert.match(fn,/versandfertiges Antwortschreiben auf \$\{referenceLanguageName\}/)
assert.match(fn,/Übersetzung genau der reference_copy/)
...
assert.match(fn,/reference_language:requestedReferenceLanguage/)
